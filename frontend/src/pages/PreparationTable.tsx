      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className={inputCls} value={form.fileNumber} onChange={(e) => set('fileNumber', e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <input className={inputCls} value={form.releasePoint} onChange={(e) => set('releasePoint', e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="number" min="0" className={inputCls} value={form.weight} onChange={(e) => set('weight', e.target.value)} />
          </div>
          {form.shipmentType === 'LCL' && (
            <div>
              <label className={labelCls}>נפח (CBM)</label>
              <input type="number" min="0" className={inputCls} value={form.volume} onChange={(e) => set('volume', e.target.value)} />
            </div>
          )}
          <div className="col-span-2 flex items-center gap-2">
            ...
            />
            <label htmlFor="isDangerous" className="text-sm text-gray-700">סחורה מסוכנת</label>
          </div>
        </div>
...
export default function PreparationTable() {
  ...
  const HEADERS = [
    'מספר תיק', 'נקודת שחרור', 'סוג משלוח', 'סחורה מסוכנת',
    'כמות', 'משקל (ק"ג)', 'נפח (CBM)', 'יעד',
    'רשימת תכולה', 'פעולות', 'שליחה',
  ];
                ...
                <tbody className="divide-y divide-gray-100">
                  {shipments.map((s) => (
                    <tr key={s._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        {s.isDangerous ? (
                          <span className="px-2 py-0.5 rounded text-xs font-medium bg-red-50 text-red-700">⚠ כן</span>
                        ) : (
                      ...
                      <td className="px-4 py-3 text-gray-700 max-w-[160px] truncate" title={s.destination}>
                        {s.destination}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <input
                            ...
                            ref={(el) => { fileInputRefs.current[s._id] = el; }}
                            onChange={(e) => {
                      ...
                      <td className="px-4 py-3">
                        ...
                        {errorMap[s._id] && errorMap[s._id] !== 'שגיאה בהעלאת הקובץ' && (
                          <p className="text-xs text-red-500 mt-1 whitespace-nowrap">{errorMap[s._id]}</p>
                        )}
                      </td>
