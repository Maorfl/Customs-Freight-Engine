...
export default function CarrierManagement() {
  ...
  const loadCarriers = useCallback(async () => {
    try {
      ...
      showToast('שגיאה בטעינת הספקים', false);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handlePriceList(carrierId: string, file: File) {
    ...
    try {
      ...
      setCarriers((prev) =>
        prev.map((c) => (c._id === carrierId ? result.carrier : c))
      );
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['שם המוביל', 'ח.פ.', 'אימייל', 'מיקום', 'סוג', 'רשימת מחירים', 'פעולות'].map((h) => (
              ...
              >
                ...
                <div className="px-6 py-5 space-y-4">
                  ...
                  <div>
                    ...
                    />
                    {formErrors.hpNumber && <p className="mt-1 text-xs text-red-500">{formErrors.hpNumber}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        value={form.location}
                        onChange={(e) => setForm((p) => ({ ...p, location: e.target.value as CarrierFormData['location'] }))}
                    ...
                    <div>
                      ...
                      <select
                        value={form.type}
                        onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as CarrierFormData['type'] }))}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      >
                ...
                <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
                  ...
                  >
                    {saving ? 'שומר...' : editing ? 'שמור שינויים' : 'הוסף מוביל'}
                  </motion.button>
                </div>
              </motion.div>
            </div>
